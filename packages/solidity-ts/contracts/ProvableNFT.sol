...
contract ProvableNFT is ERC721Upgradeable, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  modifier onlyManager(uint16 nftType) {
    if (((nftType > 0 && hasRole(getManagerRole(nftType), msg.sender)) || hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) == false) {
  ...
  function mint(
    ...
    return _mint(_to, _uri, _nftDataHash);
  }
  ...
  function mintPermissioned(
    ...
    bytes32 dataHash = sha256(abi.encode(_nftData));
    ...
    _mint(_to, _nftData.nftUri, dataHash);
  }

  function proveNFTData(uint256 _tokenId, NFTData memory _nftData) public view returns (NFTData memory data) {
    _requireMinted(_tokenId);
    if (keccak256(abi.encode(_nftData)) != bytes32(_tokenId)) revert BAD_DATAHASH(keccak256(abi.encode(_nftData)), bytes32(_tokenId));
  ...
  function getManagerRole(uint16 _eventType) public pure returns (bytes32 roleHash) {
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, nftDatas[tokenId].nftUri)) : nftDatas[tokenId].nftUri;
  }

...
describe('DirectPaymentsFactory', () => {
  ...
  const fixture = async () => {
    ...
    const dpimpl = await (await ethers.getContractFactory('DirectPaymentsPool')).deploy();
    const nftimpl = await (await ethers.getContractFactory('ProvableNFT')).deploy();
    const x = await upgrades.deployProxy(f, [signer.address, dpimpl.address, nftimpl.address], { kind: 'uups' });
    factory = x as DirectPaymentsFactory;
    return x;
    // return (factory = (await upgrades.deployProxy(f, [signer.address, dpimpl.address, nftimpl.address], { kind: 'uups' })) as DirectPaymentsFactory);
  };

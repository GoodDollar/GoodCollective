...
import { DirectPaymentsPool, ProvableNFT, UBIPool, UBIPoolFactory } from 'typechain-types';
...
describe.skip('Trust Superapp', () => {
  ...
  before(async () => {
    ...
    sf = await frameworkDeployer.getFramework();
    ...
    gdframework = await deploySuperGoodDollar(signers[0], sf, [
  ...
  const fixture = async () => {
    ...
    const dpimpl = await ethers.deployContract('TrustPool', [sf['host']], {

...
function WalletProfile({ imageUrl, firstName, lastName, donor, steward, type }: WalletProfileProps) {
  // TODO: How to know number of people supported?
  ...
  const donations: number = donor ? parseFloat(ethers.utils.formatEther(donor.totalDonated)) : 0;
        ...
        <View style={styles.profileContentBox}>
          <View style={[styles.container, styles.desktopContainer]}>
            ...
            <WalletDetails
              ...
              firstName={firstName}
              lastName={lastName}
              actionsPerformed={steward?.actions ?? 0}
              amountReceived={usdValue}
              collectivesTotal={collectivesTotal}
              creationDate={creationDate}
              amountDonated={formattedDonations}
              ...
              {[]?.map((t: any) => (
                <View style={styles.collectiveCardsContainer}>
                  <CollectiveCard
                    ...
                    formattedTotal={formattedDonations}
                    formattedUsd={usdValue}
                    donor={true}
                    donateSubPath={''}
                  />
                </View>
